...
export default function CartPage() {
  ...
  useEffect(() => {
    // All localStorage access is here — only runs on client, never on server
    const name  = localStorage.getItem("customerName") || "Guest";
    const table = localStorage.getItem("tableNumber")  || "";
    const key   = name && table ? `currentCart_${table}_${name}` : null;

    setCustomerName(name);
    setCartKey(key);          // save key in state so mutations always use the same key
  ...
  // Always writes using the key we read on mount — no state race possible
  ...
  const syncToBackend = async (items: CartItem[]) => {
    ...
    try {
      await fetch(`${BASE_URL}/api/customer/cart`, {
        ...
        body: JSON.stringify({ items: items.map(i => ({ item_id: i.id, qty: i.qty })) }),
      });
  ...
  const decQty = (id: string) => {
    const updated = cart.map(i => i.id === id ? { ...i, qty: i.qty - 1 } : i).filter(i => i.qty > 0);
    persistCart(updated);
    syncToBackend(updated);
  };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .back-btn{display:flex;align-items:center;gap:6px;padding:9px 16px;border-radius:11px;border:none;cursor:pointer;background:rgba(12,2,4,.52);border:1.5px solid rgba(200,169,81,.38);color:#fff;font-family:var(--sans);font-size:13px;font-weight:600;-webkit-tap-highlight-color:transparent;transition:all .25s;flex-shrink:0;backdrop-filter:blur(8px);}
        ...
        .sync-pill{flex-shrink:0;display:flex;align-items:center;gap:5px;padding:5px 10px;border-radius:20px;background:rgba(200,169,81,.12);border:1px solid rgba(200,169,81,.25);font-family:var(--sans);font-size:10px;color:rgba(200,169,81,.7);}
        @keyframes spin{to{transform:rotate(360deg);}}
        .sync-dot{width:6px;height:6px;border-radius:50%;border:1.5px solid rgba(200,169,81,.4);border-top-color:var(--g);animation:spin .6s linear infinite;}
        ...
        .empty{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:55vh;gap:16px;animation:fadeUp .6s both;}
        .empty-icon{width:96px;height:96px;border-radius:50%;background:linear-gradient(135deg,rgba(200,169,81,.15),rgba(212,183,110,.1));border:2px solid rgba(200,169,81,.25);display:flex;align-items:center;justify-content:center;}
        ...
        .empty-btn{padding:12px 28px;border-radius:12px;border:none;cursor:pointer;background:linear-gradient(135deg,var(--cm),var(--c));color:var(--g);font-family:var(--sans);font-size:13px;font-weight:700;transition:transform .3s,box-shadow .3s;box-shadow:0 6px 20px rgba(93,22,22,.25);}
        ...
        .cart-card{background:var(--card);border:1.5px solid rgba(200,169,81,.2);border-radius:18px;overflow:hidden;opacity:0;transform:translateY(20px);animation:fadeUp .5s both;transition:box-shadow .35s,border-color .35s,transform .35s;}
        .cart-card:hover{border-color:rgba(200,169,81,.45);box-shadow:0 12px 40px rgba(93,22,22,.1);transform:translateY(-2px);}
        ...
        .item-img{flex-shrink:0;width:84px;height:84px;border-radius:12px;overflow:hidden;border:1.5px solid rgba(200,169,81,.3);position:relative;}
        @media(min-width:768px){.item-img{width:96px;height:96px;}}
        ...
        .del-btn{flex-shrink:0;width:30px;height:30px;border-radius:8px;border:none;cursor:pointer;background:rgba(185,28,28,.08);color:#b91c1c;display:flex;align-items:center;justify-content:center;-webkit-tap-highlight-color:transparent;transition:background .2s,transform .25s;}
        ...
        .qwrap{display:flex;align-items:center;height:34px;border-radius:10px;overflow:hidden;background:rgba(200,169,81,.1);border:1.5px solid rgba(200,169,81,.35);}
        .qbtn{width:34px;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--cm),var(--c));color:var(--g);font-size:17px;font-weight:700;border:none;cursor:pointer;-webkit-tap-highlight-color:transparent;flex-shrink:0;}
        ...
        .total-card{background:linear-gradient(135deg,rgba(200,169,81,.12),rgba(212,183,110,.08));border:1.5px solid rgba(200,169,81,.35);border-radius:18px;padding:18px 20px;margin-top:20px;margin-bottom:24px;animation:fadeUp .6s .25s both;}
        .total-row{display:flex;justify-content:space-between;font-family:var(--sans);font-size:13px;color:rgba(93,22,22,.7);padding:4px 0;}
        ...
        .cta-wrap{position:fixed;bottom:0;left:0;right:0;z-index:40;padding:10px 16px 22px;background:linear-gradient(to top,rgba(250,245,236,.98) 65%,transparent);backdrop-filter:blur(4px);}
        @media(min-width:768px){.cta-wrap{padding:12px 32px 24px;}}
        .cta-inner{max-width:860px;margin:0 auto;}
        .proceed-btn{width:100%;display:flex;align-items:center;justify-content:space-between;padding:15px 22px;border-radius:16px;border:1.5px solid rgba(200,169,81,.42);cursor:pointer;background:linear-gradient(135deg,#6E1A1A,#4A1010);box-shadow:0 8px 32px rgba(45,10,15,.4);-webkit-tap-highlight-color:transparent;transition:transform .3s,box-shadow .3s;}
        .proceed-btn:hover{transform:translateY(-2px);box-shadow:0 12px 40px rgba(45,10,15,.5);}
      ...
      <div className="page" style={{ paddingBottom: cart.length > 0 ? 96 : 28 }}>
        <header className="hdr">
          ...
          <div className="hdr-inner">
            ...
            {syncing ? <div className="sync-pill"><div className="sync-dot" />saving</div> : <div className="hdr-spacer" />}
          </div>
          <div className="gl2" />
        </header>
        ...
        <div className="body" style={{ paddingTop: 20 }}>
          ...
          {cart.length > 0 && (
            <div className="total-card">
              <p style={{ fontFamily:"var(--sans)", fontSize:11, fontWeight:600, color:"rgba(200,169,81,.65)", letterSpacing:".16em", textTransform:"uppercase", marginBottom:12 }}>Order Total</p>
              {cart.map(item => (
                <div className="total-row" key={item.id}>
                  <span>{item.name} × {item.qty}</span>
                  <span style={{ fontWeight:600, color:"var(--c)" }}>₹{(parseFloat(item.price) * item.qty).toFixed(0)}</span>
                </div>
              ))}
